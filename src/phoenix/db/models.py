...
class User(HasId):
    ...
    # Raw OIDC `groups` claim as of this user's most-recent full login --
    # wholesale-replaced (not merged) on every login, and authoritative for
    # the life of the session (including across Phoenix's own token
    # refreshes, which never re-contact the IdP). Project access is computed
    # live from this list against the group->project mapping config -- see
    # phoenix.server.access.resolution -- rather than pre-materialized into
    # its own grant table.
    idp_groups: Mapped[list[str]] = mapped_column(JsonList, nullable=False, server_default="[]")
