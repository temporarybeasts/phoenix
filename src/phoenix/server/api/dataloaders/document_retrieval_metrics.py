...
Key: TypeAlias = tuple[RowId, ProjectId, EvalName, NumDocs]
...
class DocumentRetrievalMetricsDataLoader(DataLoader[Key, Result]):
    ...
    async def _load_fn(self, keys: list[Key]) -> list[Result]:
        keys_by_project: defaultdict[ProjectId, list[Key]] = defaultdict(list)
        for key in keys:
            keys_by_project[key[1]].append(key)
        ...
        for project_id, project_keys in keys_by_project.items():
            ...
            all_row_ids = {row_id for row_id, _, _, _ in project_keys}
            stmt = stmt.where(mda.span_rowid.in_(all_row_ids))
            all_eval_names = {eval_name for _, _, eval_name, _ in project_keys}
            if None not in all_eval_names:
                stmt = stmt.where(mda.name.in_(all_eval_names))
            max_position = max(num_docs for _, _, _, num_docs in project_keys)
            ...
            for row_id, _, eval_name, num_docs in project_keys:
            ...
            async with project_scoped_read_connection(self._db, project_id) as session:
                ...
                async for (span_rowid, name), group in groupby(
                    data, lambda r: (r.span_rowid, r.name)
                ):
                    ...
                    for eval_name in (name, None):
                        for num_docs in requested_num_docs.get((span_rowid, eval_name)) or ():
                            ...
                            key = (span_rowid, project_id, eval_name, num_docs)
